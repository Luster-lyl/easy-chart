<template>
  <VueDragResize :isActive="candleStickState.clickState" :w="candleStickState.w" :h="candleStickState.h"
                 :x="candleStickState.x" :y="candleStickState.y"
                 v-on:resizing="resize" v-on:dragging="resize">
<!--    v-on:dragstop="resize">-->
    <div :id="id" :style="{'width':candleStickState.w+'px','height':candleStickState.h+'px'}"></div>
  </VueDragResize>
</template>
...
export default {
  ...
  data(){
    return {
      candleStickSize: {
        idKey: this.$props.candleStickState.idKey,
        bigScreenIdKey:this.$props.bigScreenId,
        typeName:'candleStickState',
        width: 0,
        height: 0,
        top: 0,
        left: 0
  ...
  methods:{
    ...
    resize(newRect) {
      this.$data.candleStickSize.width = newRect.width;
      this.$data.candleStickSize.height = newRect.height;
      this.$data.candleStickSize.top = newRect.top;
      this.$data.candleStickSize.left = newRect.left;
      this.$store.commit('resizeState',this.$data.candleStickSize)
      this.myChartData.resize({width:this.$data.candleStickSize.width,height:this.$data.candleStickSize.height})
      // store只能传递一个参数？？？。。？
    },
  }
}
